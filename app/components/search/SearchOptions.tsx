...
export default function SearchOptions({ optionsState, visible } : { optionsState: CardParameters[], visible: boolean }) {
    ...
    return (
        <div 
            className={`flex flex-col w-[90%] max-w-[500px] cursor-pointer z-1 absolute overflow-y-scroll max-h-52 mt-[6.5rem] rounded-xl border-2 bg-white bg-opacity-90 ${optionsState.length > 0 && (visible || hovered) ? "visible" : "invisible"}`}
